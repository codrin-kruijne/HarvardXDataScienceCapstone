...
```{r}
## Amount of ratings
rating_counts <- rating_data %>% group_by(userId) %>% count()
user_data <- user_data %>% inner_join(rating_counts)

## Ratings statistics
rating_stats <- rating_data %>% group_by(userId) %>%
                                mutate(user_min = summary(rating)[1]) %>%
                                mutate(user_firstq = summary(rating)[2]) %>%
                                mutate(user_median = summary(rating)[3]) %>%
                                mutate(user_mean = summary(rating)[4]) %>%
                                mutate(user_thirdq = summary(rating)[5]) %>%
                                mutate(user_max = summary(rating)[6]) %>%
                                select(userId, user_min, user_firstq, user_median, user_mean, user_thirdq, user_max) %>%
                                unique()
user_data <- user_data %>% inner_join(rating_stats)
user_data <- rename(user_data, c("n" = "user_ratings"))
...
movie_ratings <- rating_data %>% group_by(movieId) %>% count()
movie_data <- movie_data %>% inner_join(movie_ratings)

# Movie rating statistics
movie_stats <- rating_data %>% group_by(movieId) %>%
                               mutate(movie_min = summary(rating)[1]) %>%
                               mutate(movie_firstq = summary(rating)[2]) %>%
                               mutate(movie_median = summary(rating)[3]) %>%
                               mutate(movie_mean = summary(rating)[4]) %>%
                               mutate(movie_thirdq = summary(rating)[5]) %>%
                               mutate(movie_max = summary(rating)[6]) %>%
